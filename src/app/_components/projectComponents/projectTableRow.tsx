...
export default function ProjectTableRow({project}: {project: createProjectType}) {
    ...
    const tableEntry = organization ? (
        ...
        <TableCell data-cy={`project-table-row-${project.name}`} className="font-medium pl-10">{project.name}</TableCell>
        <TableCell data-cy={`project-table-row-${project.description}`} className="text-right">{project.description}</TableCell>
